...
let rec processCommands system (actorGroups:ActorGroups) userId = 
    let loop () = processCommands system actorGroups userId
    printfn """
...
5) ...
    ...
    | true, 3 ->
        processDate (fun date ->          
            let meeting = Persistence.ClubMeetings.findByDate date
            let placements = Persistence.RolePlacements.findMeetingPlacements meeting.Id  
            editMeeting system actorGroups userId meeting placements)
        loop ()        
    | true, 4 -> 
